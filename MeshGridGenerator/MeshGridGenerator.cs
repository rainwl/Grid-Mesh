...
public class MeshGridGenerator : MonoBehaviour
{
    ...
    public UnityEngine.Vector3 cellSize = new UnityEngine.Vector3(0.2f, 0.2f, 0.2f);
    ...
    private void Start()
    {
        ...
        var cellCenters = new NativeArray<UnityEngine.Vector3>(gridSize.x * gridSize.y * gridSize.z, Allocator.TempJob);
        ...
        for (var x = 0; x < gridSize.x; x++)
        {
            for (var y = 0; y < gridSize.y; y++)
            {
                ...
                {
                    var cellCenter = gridStart + new UnityEngine.Vector3(x * cellSize.x, y * cellSize.y, z * cellSize.z);
                    cellCenters[x + y * gridSize.x + z * gridSize.x * gridSize.y] = cellCenter;
                }
            }
        }

        var meshVertices = new NativeArray<UnityEngine.Vector3>(mesh.vertices, Allocator.TempJob);
